        ...
        if model_path.exists():
            try:
                ...
                if model_inference.loaded:
                    ...
                    logger.warning(
                        "Model file exists but failed to load. Using mock predictions."
                    )
...
async def predict_batch(
    files: List[UploadFile] = File(...), db=Depends(get_db)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for file in files:
            try:
                ...
                result["confidence"] = float(result.get("confidence", 0.0))  # type: ignore[arg-type]
                result["probabilities"] = [float(x) for x in result.get("probabilities", [])]  # type: ignore[attr-defined]
                result["timestamp"] = str(result.get("timestamp", ""))
                result["model_status"] = (
                    str(result.get("model_status", ""))
...
async def get_model_status() -> Dict[str, Any]:
    ...
    status_info = {
        ...
        and hasattr(model_inference, "loaded")
        and model_inference.loaded,
...
async def reload_model(model_path: str, model_type: str = "keras") -> Dict[str, Any]:
    """
    Reload ML model
